...
This is a broader search of find -name. This allows the user to search up specific phrases of the file name they are looking for and ensures that they do not have to worry about being case-sensitive.

**Example 1:** `find technical// -iname "chap*"`
...
```
Here I am able to find all the files containing the word "chapter" in them even though the phrase I searched up was "chap
". This allows for the user to search up and find something even if they do not know the entirety of what they are looking for.

**Example 2:** `find technical// -iname "protocol*"`
```
technical///government/About_LSC/Protocol_Regarding_Access.txt
```
Here only one file is outputted. This was the only file with the word "protocol" in it. However, the word in our search did not account for capitalization. Using the -iname command, we are able to broaden the search and not restrict it to be case sensitive—in which case we would not have found this file.




**Example 3**: `find technical// -iname "state*"`
...
```
Here, using the regular -name command would not have returned anything. However, due to -iname, we are able to search beyond just the search arguments and look into phrases and case insensitive results to procure these results.

### Option 2: find -size n

This allows the user to search for files within a specific size range (n). The default is bytes. But through adding a suffix such as k, M, G it allows to search through n scaled to be as kilobytes, megabytes, gigabytes, etc.


**Example 1:** `find technical// -size -10k `
...
This list is super long, but essentially has returned to us all the files that are under 10 kilobytes.

**Example 2:** `find technical// -size +200 `
...
technical///911report/chapter-12.txt
```
Here we used "+", in the command instead. This made it so that the terminal returned all the files above 200 bytes.

**Example 3**: `find technical// -size -2`
...
technical///government/Post_Rate_Comm
```
For this, I played around with the numbers, and tried to see how low I could go and still get an output. Using the argument -2, I was able to find the smallest files within the technical directory.

### Option 3: find -atime n or find -amin n

This command allows for the user to search for files that have been/not been accessed in n amount of days or minutes.

The "+" means the file has not been accessed in n or more amount of time. The "-" means the file has been accessed in n amount of time or less.

**Example 1:** `find technical// -atime +7`
...
This returned all the files that have not been accessed in more than 7 days.

**Example 2:** `find technical// -amin -30`
...
This command used "-amin" so the output is all the files that were accessed in the last 30 minutes

**Example 3:** `find technical// -atime -1`
...
This command made use of "-atime" so it essentially returned all of the files that were accessed in the last day. Probably the ones that I've been opening up while writing this lab report!